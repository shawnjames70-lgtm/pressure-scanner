...
_live_data = {
    "auth_state": "Waiting to connect...",
    "price": 0.0,
    "vwap": 0.0,
    "tick": 0,
    "add": 0,
    "candles": {"1m":[], "5m":[], "15m":[], "30m":[]},
    "volumes": {"1m":0, "5m":0, "15m":0, "30m":0},
    "rvol": {"1m":0, "5m":0, "15m":0, "30m":0},
    "connected": False
}
...
.step-pass{
  ...
  color:#4d9fff;font-weight:bold;font-family:Arial,Helvetica,sans-serif;
  box-shadow:inset 0 0 20px #0066ff11;}
.step-fail{
  background:linear-gradient(90deg,#1a1200,#221800);
  border-left:5px solid #ffd700;
  padding:12px 16px;border-radius:8px;margin:5px 0;
  color:#ffd700;font-weight:bold;font-family:Arial,Helvetica,sans-serif;
    ...
    for evt in data:
        ...
        if etype == "Candle" and channel in ch_map:
            try:
                ...
                _eval_signal(get_data()["tick"], tick_thr)
...
<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0 4px;">
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#555;">
...
with m3:
    ...
    cl1m = list(get_data()["candles"]["1m"])[-8:]
    if cl1m:
...
<div style="text-align:center;color:#1a2233;font-size:10px;padding:16px 0 4px;
  font-family:Arial,Helvetica,sans-serif;border-top:1px solid #0d1117;margin-top:16px;">
